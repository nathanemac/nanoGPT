...
METHOD_CONFIG_MAP["sczo"]="train_sczo_config.py"
...
print_usage() {
    ...
    echo "  $0 sczo single --sczo_alpha=5e-4 --sczo_bootstrap_interval=30"
    echo ""
    ...
    echo "  - sczo     : SCZO (Secant-Constrained Zero-Order) method (uses train_sczo_config.py)"
    echo "               Novel method using secant constraints to update gradient estimates"
    echo "               Only requires 2 forward passes per iteration, no backward passes"
